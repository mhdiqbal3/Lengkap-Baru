...
<head>
    ...
    <title>Buat Password Baru - SATGAS PPKS USN Kolaka</title>
...
<body class="bg-[#800000] flex min-h-screen items-center justify-center p-4 font-sans">
    <div class="bg-white w-full max-w-md rounded-[2rem] shadow-2xl p-8 border border-gray-100">
        ...
        @if (session('success'))
            <div
                class="bg-green-50 text-green-700 p-3 rounded-xl text-xs font-bold border border-green-200 mb-6 text-center">
                {{ session('success') }}
            </div>
        @endif

        <div class="text-center mb-6">
            <h1 class="text-2xl font-extrabold text-gray-900 tracking-tight mb-2">Buat Sandi Baru</h1>
            <p class="text-xs font-medium text-gray-500">Silakan masukkan password baru untuk akun Anda.</p>
        </div>
        ...
        <form method="POST" action="{{ route('password.update') }}" class="space-y-4">
            ...
            <div class="space-y-1.5" x-data="{ show: false }">
                ...
                <div class="relative">
                    ...
                    <button type="button" @click="show = !show"
                        class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#800000] p-1">
            ...
            <button type="submit"
                class="w-full py-3 mt-3 bg-[#800000] text-white text-sm font-bold rounded-xl shadow-lg hover:bg-[#600000] transition-all active:scale-[0.98]">
